-- Add specialization field to artists and profiles tables
...
-- Update the comprehensive profile creation function to handle specialization
  ...
  IF NEW.user_type = 'artist' AND (OLD.user_type IS NULL OR OLD.user_type != 'artist') THEN
    IF NOT EXISTS (SELECT 1 FROM public.artists WHERE profile_id = NEW.id) THEN
      INSERT INTO public.artists (
        ...
        availability_status, rating, total_reviews, experience_years, specialization
      ) VALUES (
        NEW.id,
